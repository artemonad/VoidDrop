<script lang="ts">
    ...
    let total = $derived(s.receiverFileTotalSize);
    let actualTransferred = $derived(Math.min(s.bytesTransferred, total));
    ...
    {:else if s.flowState === "STREAMING"}
        <div class="status-box status-active">
            ...
            <div class="current-transfer-info">
                {#if s.receiverManifest}
                    {@const currentFile = s.receiverManifest.files[s.receiverFileIndex]}
                    {#if currentFile}
                        <p>
                            Saving: <span style="font-family: monospace; word-break: break-all;">{currentFile.path}</span>
                            <span style="color: var(--blue); font-weight: bold;"> &rarr; </span>
                            <span>
                                {s.receiverSaveLocationName || 'none'}
                            </span>
                        </p>
                        <p style="margin-top: 0.25rem;">File {s.receiverFileIndex + 1} of {s.receiverManifest.files.length}</p>
                    {/if}
                {:else}
                    <p>Processing payload stream in real-time...</p>
                {/if}
            </div>
